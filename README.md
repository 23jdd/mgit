...
`mgit add .`、`mgit status` 的未跟踪文件扫描、`mgit write-tree --worktree` 和 `mgit commit --worktree` 都会读取仓库根目录的 `.gitignore`。当前支持空行、`#` 注释、`!` 反选、目录规则、根路径规则和 `*` / `?` 通配；`.git`、`.mygit`、`.gocache`、`.agents`、`.codex` 始终会被跳过。
...
go run . stash push -m "try something"

# 查看 stash 列表
go run . stash list

# 应用但保留 stash
go run . stash apply stash@{0}

# 应用并删除 stash
go run . stash pop

# 删除 stash
go run . stash drop stash@{0}
```
...
`reflog` 读取 `.mygit/logs/HEAD`，会记录 `commit`、`reset`、`merge` 等通过 `updateHead` 移动 HEAD 的操作。
...
当前 index 只记录普通文件，路径统一使用 `/`。执行 `add .` 时会跳过 `.gitignore` 匹配的路径，以及 `.git`、`.mygit`、`.gocache`、`.agents` 和 `.codex`。
